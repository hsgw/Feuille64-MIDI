...
inline struct cRGB rgb_init(uint8_t r, uint8_t g, uint8_t b) {
    struct cRGB rgb;
    rgb.r = r;
    rgb.g = g;
    rgb.b = b;
...
inline struct cRGB rgb_init_brightness(uint8_t brightness) {
    struct cRGB rgb;
...
void leds_init(void) {
    for (uint8_t i = 0; i < LED_COUNT; i++) {
        leds_rgb[i] = rgb_init(0, 0, 0);
    }
}

void leds_update(void) { ws2812_setleds(leds_rgb, LED_COUNT); }

void leds_set(uint8_t row, uint8_t col, uint8_t brightness) { leds_rgb[LED_COL * row + (row % 2 == 0 ? col : LED_COL - 1 - col)] = rgb_init_brightness(brightness); }
